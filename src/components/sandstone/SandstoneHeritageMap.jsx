...
export const SandstoneHeritageMap = () => {
  ...
  return (
    <section id="map" className="py-24 px-4 sm:px-8 bg-gradient-to-b from-[#1B120C] via-[#24170F] to-[#1B120C] relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative w-full h-[520px] rounded-3xl bg-[#24170F]/90 border border-[#B9854F]/40 shadow-2xl p-6 overflow-hidden flex flex-col justify-between">
          ...
          <div className="absolute top-6 right-6 opacity-20 pointer-events-none">
            <Compass size={120} className="text-[#D4A66A] animate-spin duration-[60000ms]" />
          </div>
          ...
          {mapNodes.map((node) => {
            ...
            return (
              ...
              >
                ...
                {/* Node Label Tooltip */}
                <div className={`absolute left-1/2 -translate-x-1/2 bottom-10 px-3 py-1 rounded-md bg-[#1B120C] border border-[#B9854F]/50 text-[11px] font-mono text-[#F5E7CF] whitespace-nowrap transition-all pointer-events-none shadow-lg ${
          ...
          <div className="relative z-30 max-w-md p-5 rounded-2xl bg-[#1B120C]/90 backdrop-blur-xl border border-[#D4A66A]/40 shadow-2xl">
            {mapNodes.filter(n => n.id === activeLocation).map(node => (
              <div key={node.id} className="flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] uppercase font-mono tracking-widest text-[#D4A66A] px-2 py-0.5 rounded bg-[#342117] border border-[#B9854F]/30">
                    {node.region}
                  </span>
                  <span className="text-xs text-[#E8CFA8]/60 font-mono">NODE // {node.id.toUpperCase()}</span>
                </div>
                <h4 className="font-serif-heritage text-2xl font-bold text-[#F5E7CF]">
                  {node.name}
                </h4>
          ...
          <div className="relative z-30 flex justify-end items-center gap-4">
            <a
              href="#planner"
              className="px-6 py-3 rounded-full bg-gradient-to-r from-[#6B4325] to-[#8B5E34] hover:from-[#8B5E34] hover:to-[#B9854F] text-[#F5E7CF] text-xs font-semibold tracking-widest border border-[#D4A66A]/40 shadow-xl flex items-center gap-2"
            >
